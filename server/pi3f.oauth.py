...
DEGRADED_SIZE = 400
DEGRADED_QUALITY = ""
# DEGRADED_QUALITY = "gray"
# DEGRADED_QUALITY = "bitonal"
...
class ImageApp(object):
    ...
    def make_info(self, infoId, image):
        ...
        if infoId.endswith('-degraded'):
            ...
            if imageW > imageH:
                ratio = float(DEGRADED_SIZE) / imageW
                imageH = int(imageH * ratio)
                imageW = DEGRADED_SIZE
            else:
                ratio = float(DEGRADED_SIZE) / imageH
                imageW = int(imageW * ratio)
                imageH = DEGRADED_SIZE
            all_scales = [1,2]
        else:
            all_scales = [1,2,4,8,16]            

        if image.mode == '' or DEGRADED_QUALITY == 'bitonal':
            qualities = [] 
        elif image.mode == 'L' or DEGRADED_QUALITY == 'gray':
            qualities = ['gray']     
        else:
    ...
    def handle_GET(self, path):
        ...
        if os.path.exists(fp):
            ...
            response['Link'] += ', <%s%s>;rel="canonical"' % (BASEPREF, fp)
